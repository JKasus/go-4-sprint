...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		distance := distance(action)                               // вызовите здесь необходимую функцию (km)
		speed := meanSpeed(action, duration)                       // вызовите здесь необходимую функцию (km/h)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	result := ((walkingCaloriesWeightMultiplier*weight + ((math.Pow(speed, 2))/heightM)*walkingSpeedHeightMultiplier*weight) * duration * minInH)
	return result
}
